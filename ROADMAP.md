...
## PHASE 16.10 — Clinical Term Autocomplete & Library ✅ COMPLETE (seed lists shipped ~2,400 terms; learned layer now syncs per-hospital via the `clinical_terms` table — see the 2026-07-25 update log)
...
* 2026-07-25: **The Overhaul (Stages 1–8) — audit-driven hardening of everything.** A full audit
  (persistence, features, scalability, code health) was turned into an 8-stage program and executed
  end-to-end. **Stage 1 · Visibility:** app-wide FR/EN toast system (`lib/notify.ts` + `Toaster`),
  guarded storage writes, a **sync health panel** (waiting / retrying / needs-attention with recorded
  errors + retry/discard actions), and a friendly "backend not configured" screen + `.env.example`.
  **Stage 2 · Stop data loss:** billing tables joined the optimistic-concurrency guard; conflicts are no
  longer silently dropped — the losing edit is kept, the user notified, and one tap re-applies it;
  retry cap + exponential backoff + dead-letter; cross-tab drain lock (Web Locks) + storage events;
  child-first delete ordering; hydrate re-applies pending edits; corrupt-outbox backup; and the learned
  clinical-term layer became a real synced `clinical_terms` table (device blob migrated + uploaded).
  **Stage 3 · Storage engine:** localStorage blob → **in-memory authoritative cache + IndexedDB
  per-row mirror** (`services/db/engine.ts`, `services/localDb.ts`), BroadcastChannel cross-tab
  convergence, cache-schema migration framework, reactive `useCacheVersion()` wired into every screen,
  and a sign-out privacy wipe (drain first; unsent work preserved + announced). **Stage 4 · Smart
  sync:** hydration is **windowed** (open work + last 12 months; patients/structural tables in full;
  older history fetched on demand per patient and cached), **Supabase Realtime** streams colleagues'
  changes into every device (with pending-edit protection + reconnect backfill), lab-file uploads queue
  offline (`pending_uploads` IndexedDB store) and upload on reconnect, reports fetch server rows for
  ranges beyond the window (with an offline partial-data notice), and global search falls back to the
  server. **Stage 5 · Feature repairs:** the drawer now uses the redesigned reconciliation dialog;
  vitals get clinical range validation (SpO₂/BP/pulse/temp/weight/GCS) and intake rejects future DOBs;
  the fake follow-up console stubs became a real **`/follow-ups` task list** backed by a
  `follow_up_tasks` table (2 tasks per discharge: day-2 call, day-7 check-in); staff can be **edited,
  deactivated, and password-reset** (new admin-verified server action); departments archive with
  confirm; the offline page is bilingual; and the drawer gained a **cross-visit patient timeline**.
  **Stage 6 · Security & scale:** RLS helpers wrapped for InitPlan evaluation + JWT-claim-first
  resolution (optional auth hook documented); `is_hospital_active()` + a subscription **write-gate
  trigger** on every domain table (suspended = read-only); `prune_audit_log()` retention function;
  `provisionStaffLogin` now authenticates its caller (it previously had **no check at all**) and the
  legacy `provisionHospital` action is disabled in production; reusable `RoleGate`; and
  **`docs/PRODUCTION-CUTOVER.md`** consolidates every manual go-live step. **Stage 7 · Code health:**
  `mockStorage.ts` (4,600 lines) split into `services/db/*` behind a 34-line barrel; the 2,155-line
  `PatientDrawer` decomposed into 13 section components (485-line orchestrator) — also fixing a
  latent death-confirm state leak across patients; the five big pages slimmed (billing 828→293,
  staff 844→184, floor-map 656→153, care-plans 747→232, intake 555→253) onto shared
  `StatusBadge`/`FormDialog` primitives; **branded id types** (a VisitId can no longer pass as a
  PatientId); enums derive from `as const` arrays with a **schema↔types parity test** (drift fails
  CI); i18n keys are **compile-time checked** (`MessageKey` union — a typo'd key no longer builds);
  `xlsx` swapped to the patched SheetJS 0.20.3 vendor build (0 advisories); and a new **jsdom test
  project** (`npm run test:dom`, Testing Library) covers the toaster, sync panel, form dialog, status
  badge, and vitals validation. **Stage 8 · Docs:** README architecture rewritten, this entry, and the
  Demographics/ROS design doc's paths corrected to the flat repo layout (still unimplemented by
  choice). Verify gate: `tsc` clean, **276 node tests + 14 DOM tests** green, `next build` green.
  Deliberate decisions recorded: no dedicated API tier (direct Supabase + RLS scales; revisit for
  cross-tenant analytics), no server-side page gating (offline-first pages must render without a
  server — RLS is the data boundary, `RoleGate` the UX layer), SMS follow-ups deferred in favor of the
  in-app task list. **Not run here:** the Docker-based integration suites (`test:rls` etc.) — run them
  with the local stack up before shipping, and apply the new schema/snippets to any live database.
