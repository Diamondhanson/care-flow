...
* [ ] Department directory + admin management UI.
* [ ] Route a visit to a department at registration; filter the board/queues by department.
* [ ] Per-department views so each unit sees its own patients while admin sees everything.

## PHASE 8 — Clinical Encounter & Documentation 🔜

**Goal:** the doctor's actual work is captured in the record.

* [ ] Doctor consultation view: prior patient info shown on open; SOAP note entry.
* [ ] Structured diagnosis entry (ICD-10 lookup, primary/secondary).
* [ ] Disposition decision: discharge-home / admit / observation / refer.
* [ ] Patient timeline showing every consultation across visits.

## PHASE 9 — Orders & Results Loop 🔜

**Goal:** "doctor recommends test → test done → result recorded → doctor decides."

* [ ] Doctor orders tests (lab / imaging / procedure) from the consultation.
* [ ] Lab/imaging queue; lab tech enters results (with file upload to storage).
* [ ] Results surface back on the visit; abnormal-flag highlighting; doctor review.
