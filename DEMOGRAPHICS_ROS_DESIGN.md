...
**Design document** · Author: architecture pass · Date: 2026-07-02 · Status: proposal for review (rev 2)
...
| **Symptom vocabulary by body system** | `apps/web/data/clinical-terms/subjective.json` | **301 symptom terms, each tagged with a `system`, bilingual EN/FR with lay-term synonyms, across 12 systems** |
...
The app currently runs on `mockStorage` (`apps/web/services/mockStorage.ts`, ~4,100 lines) with a **parallel** Supabase schema (`packages/db/schema.sql`). Each data-model change touches four coordinated places:

1. `packages/db/schema.sql` — Postgres DDL, enums, indexes, triggers, RLS.
2. `packages/shared/types/healthcare.ts` — TypeScript interfaces.
3. `apps/web/services/mockStorage.ts` — the in-memory/localStorage implementation the UI reads today.
4. `apps/web/services/supabaseData.ts` — the Supabase-backed implementation.
...
## 5. Shared type changes (`packages/shared/types/healthcare.ts`)
...
- **Answer-shape sprawl in `answer_value` jsonb.** Constrained by `answer_type` + a `zod` validator per type in `packages/shared/validation`; `answer_label` is always the printable source of truth.
...
*Grounded in: `packages/db/schema.sql`, `packages/shared/types/healthcare.ts`, `apps/web/services/mockStorage.ts`, `apps/web/components/live-board/patient-drawer.tsx`, `apps/web/data/clinical-terms/subjective.json`, `apps/web/lib/clinical-terms/index.ts`, `apps/web/app/(app)/intake/page.tsx`.*
