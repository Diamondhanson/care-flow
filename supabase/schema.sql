...
-- a `hospital_id` FK, every RLS policy ANDs in `hospital_id = (select current_hospital_id())`,
...
-- section 6d) while reads keep working — a hospital's records stay viewable.
...
begin
  foreach t in array array[
    ...
    'care_plan_items','billable_items','charges','clinical_terms',
    'follow_up_tasks'
  ]
  loop
...
begin
  foreach t in array array[
    'hospitals','departments','wards','beds','staff','patients','visits',
    'consultations','orders','prescriptions','admissions','allergies',
    'care_plan_items','billable_items','charges','follow_up_tasks'
  ]
  loop
    execute format(
...
-- `hospital_id = (select current_hospital_id())` so a role check never leaks across
...
begin
  foreach t in array array[
    ...
    'billable_items','charges','clinical_terms','follow_up_tasks','audit_log'
  ]
  loop
...
begin
  foreach t in array array[
    ...
    'clinical_terms','follow_up_tasks'
  ]
  loop
...
-- Post-discharge follow-up tasks are worked by the clinical team.
