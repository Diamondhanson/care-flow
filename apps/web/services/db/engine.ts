...
/** Fire {@link NOTIFICATIONS_EVENT} (used by the persist hook + read-state mutators). */
...
export function persist(db: Database, opts: { track?: boolean } = {}): void {
  ...
  // Post-diff notification hook: when this persist touched notification rows
  // (queued by a mutator onto the same snapshot, or a read-state stamp), tell
  // the bell's useSyncExternalStore store to re-read — exactly the contract the
  // old engine's NOTIFICATIONS_EVENT provided. CACHE_EVENT still fires below.
  if (changes.some((c) => c.table === "notifications")) {
