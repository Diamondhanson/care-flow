/**
 * English source dictionary — the single source of truth for the app's UI copy.
 * `fr.ts` is typed `as Messages` against this object, so `tsc` fails if a key is
 * missing or renamed there (our compile-time "no English leaks" guarantee).
 *
 * Values use `{name}` placeholders for runtime interpolation (see translate()).
 * Clinical reference data (drug names, ICD-10 descriptions, GCS / SpO₂ / IV / IM,
 * and the parseable route/frequency option strings) deliberately stays canonical
 * and is NOT translated.
 */

export const en = {
  common: {
    optional: "Optional",
    save: "Save",
    saveChanges: "Save changes",
    cancel: "Cancel",
    create: "Create",
    edit: "Edit",
    add: "Add",
    remove: "Remove",
    start: "Start",
    close: "Close",
    none: "None",
    loading: "Loading…",
  },

  nav: {
    liveBoard: "Live Board",
    worklist: "Worklist",
    intake: "Register a patient",
    diagnostics: "Tests & results",
    medications: "Patient treatment guide",
    reconciliation: "Match emergency patient",
    departments: "Departments",
    floorMap: "Floor Map",
    carePlans: "Nursing care plan",
    followUps: "Follow-ups",
    billing: "Billing",
    reports: "Reports",
    askCareflow: "Ask CareFlow",
    staff: "Staff Directory",
    settings: "Settings",
  },

  shell: {
    workspace: "Workspace",
    statusBoard: "Status Board",
    facility: "General Hospital",
    liveOperations: "Live operations",
    onShift: "On shift",
    openMenu: "Open menu",
    navigation: "Navigation",
    toggleTheme: "Toggle color theme",
    switchRole: "Switch acting role (dev)",
    switchLanguage: "Switch language",
    actingRole: "Acting role",
    actingAs: "Acting as (dev)",
    noAuth: "no auth",
  },

  roles: {
    doctor: "Doctor",
    nurse: "Nurse",
    admin: "Administrator",
    lab_tech: "Lab Technician",
    pharmacist: "Pharmacist",
    receptionist: "Receptionist",
  },

  sex: {
    male: "Male",
    female: "Female",
    other: "Other",
    unknown: "Unknown",
  },

  stage: {
    registration: "Registration",
    triage: "Triage",
    consultation: "Consultation",
    diagnostics: "Diagnostics",
    treatment: "Treatment",
    discharge_planning: "Discharge planning",
    discharged: "Discharged",
    followed_up: "Followed up",
    deceased: "Deceased",
  },

  boardColumn: {
    intake: "Intake",
    consultation: "Consultation",
    treatment: "Treatment",
    discharge: "Discharge",
  },

  visitType: {
    outpatient: "Outpatient",
    inpatient: "Inpatient",
    emergency: "Emergency",
  },

  orderType: {
    lab: "Lab",
    imaging: "Imaging",
    procedure: "Procedure",
  },

  orderStatus: {
    requested: "Requested",
    in_progress: "In progress",
    completed: "Completed",
    cancelled: "Cancelled",
  },

  prescriptionStatus: {
    active: "Active",
    completed: "Completed",
    discontinued: "Discontinued",
  },

  marStatus: {
    given: "Given",
    held: "Held",
    refused: "Refused",
    missed: "Missed",
    suspended: "Suspended",
  },

  mealTiming: {
    withMeals: "With meals",
    withoutMeals: "Without meals",
    neutral: "Neutral (any time)",
  },

  doseState: {
    overdue: "Overdue",
    due: "Due now",
    upcoming: "Upcoming",
    prn: "PRN",
    inactive: "Inactive",
  },

  bedStatus: {
    free: "Free",
    occupied: "Occupied",
    reserved: "Reserved",
    cleaning: "Cleaning",
    maintenance: "Maintenance",
  },

  allergyCategory: {
    drug: "Drug",
    food: "Food",
    environmental: "Environmental",
    other: "Other",
  },

  allergySeverity: {
    mild: "Mild",
    moderate: "Moderate",
    severe: "Severe",
    life_threatening: "Life-threatening",
  },

  maritalStatus: {
    single: "Single",
    married: "Married",
    partnered: "Partnered",
    divorced: "Divorced",
    widowed: "Widowed",
    unknown: "Unknown",
  },

  historyType: {
    past_medical: "Past medical",
    past_surgical: "Past surgical",
    family: "Family",
    social: "Social",
    obstetric_gynae: "Obstetric / Gynae",
    medication: "Medications",
    immunization: "Immunizations",
  },

  ros: {
    title: "Review of Systems",
    compiledReport: "Compiled report",
    suggested: "(suggested)",
    addSystem: "Add system",
    removeSystem: "Remove {system}",
    removeSystemConfirm: "Remove {system} from this review?",
    removeSystemConfirmAnswers:
      "Remove {system}? Its {count} recorded answers will be cleared.",
    remove: "Remove",
    reviewOtherSystems: "Review other systems ({count})",
    hideOtherSystems: "Hide other systems",
    markRemainingNo: "Mark remaining as No ({count})",
    moreQuestions: "{count} more questions",
    fewerQuestions: "Fewer questions",
    historyGenetics: "History & genetics",
    answeredCount: "{count} answered",
    yes: "Yes",
    no: "No",
    addNote: "Add a note",
    notePlaceholder: "Optional note",
    textPlaceholder: "Specify…",
  },

  background: {
    title: "Background",
    ageYears: "{age} yrs",
    occupation: "Occupation",
    maritalStatus: "Marital status",
    add: "add",
    edit: "Edit",
    delete: "Delete",
    noneRecorded: "None recorded",
    noneRecordedAdd: "None recorded — tap to add",
    notRecorded: "details not recorded",
    explainer:
      "Recorded once, follows the patient across every visit — review and update rather than re-enter.",
    description: "Description",
    descriptionRequired: "A description is required.",
    saveFailed: "Could not save this record.",
    onset: "When",
    onsetPlaceholder: "e.g. 2015, childhood",
    status: "Status",
    active: "Active",
    resolved: "Resolved",
    relationLabel: "Relation",
    relation: {
      father: "Father",
      mother: "Mother",
      sibling: "Sibling",
      child: "Child",
      other: "Other",
    },
    alcohol: "Alcohol",
    alcoholLevel: {
      none: "None",
      occasional: "Occasional",
      regular: "Regular",
    },
    tobaccoPackYears: "Tobacco (pack-years)",
    packYears: "{count} pack-years",
    gravida: "Gravida",
    para: "Para",
    lmp: "LMP",
    placeholder: {
      past_medical: "e.g. Type 2 diabetes",
      past_surgical: "e.g. Appendectomy 2015",
      family: "e.g. Breast cancer",
      social: "e.g. Smokes 10/day",
      obstetric_gynae: "e.g. G2P2, no complications",
      medication: "e.g. Metformin 500 mg daily",
      immunization: "e.g. Tetanus booster 2024",
    },
  },

  rangePreset: {
    "7d": "Last 7 days",
    "30d": "Last 30 days",
    "90d": "Last 90 days",
    all: "All time",
    custom: "Custom range",
  },

  floor: {
    unassigned: "Unassigned floor",
  },

  liveBoard: {
    title: "Live Board",
    active: "active",
    subtitle: "Track every patient from admission through recovery follow-up.",
    viewing: "Viewing",
    allDepartments: "All departments",
    patients: "patients",
    noPatients: "No patients waiting here",
    unknownPatient: "Unknown patient",
    register: "Register a patient",
    emergency: "Emergency",
    openPatient: "Open {name}",
    triage: {
      label: "Priority {level}",
      aria: "Triage priority {level}: {name}",
      1: "Critical",
      2: "Emergent",
      3: "Urgent",
      4: "Less urgent",
      5: "Non-urgent",
    },
  },

  nextStep: {
    registration: "Register patient",
    triage: "Take vitals & triage",
    consultation: "Send to doctor",
    diagnostics: "Order tests",
    treatment: "Start treatment",
    discharge_planning: "Plan discharge",
    discharged: "Discharge patient",
    followed_up: "Schedule follow-up",
  },

  tour: {
    help: "Getting-started guide",
    skip: "Skip",
    back: "Back",
    next: "Next",
    done: "Got it",
    step: "Step {n} of {total}",
    welcomeTitle: "Welcome to CareFlow",
    welcomeBody:
      "A quick tour of the basics — about 30 seconds. You can replay it any time from the “?” in the top bar.",
    boardTitle: "This is your live board",
    boardBody:
      "Every patient in the hospital appears here, grouped by where they are in their care — from arrival through to discharge.",
    cardTitle: "Tap a patient to open their record",
    cardBody:
      "Each card opens the full record — notes, tests, medications, admission — and shows a “next step” hint so you always know what to do.",
    registerTitle: "Register a new arrival",
    registerBody:
      "Tap “Register a patient” at the top of the board to add someone new in under a minute.",
    switchTitle: "Switch role or language",
    switchBody:
      "Use the top bar to change your role or switch between English and French at any time.",
  },

  drawer: {
    emergency: "Emergency",
    noChiefComplaint: "No chief complaint recorded",
    allergiesCount: "Allergies ({count})",
    noKnownAllergies: "No known allergies",
    allergiesNotAssessed: "Allergies not assessed",
    reconcileTitle: "Profile reconciliation",
    reconcileHint:
      "Match this emergency record to a verified patient. Clinical logs are preserved and re-pointed to the merged profile.",
    selectVerifiedPatient: "Select verified patient",
    merge: "Merge",
    doctorConsole: "Doctor console",
    priorRecord: "Prior record",
    primaryTag: "primary",
    noConsultationNote: "No consultation note recorded yet.",
    noPriorRecord: "No prior consultations or diagnoses on this visit.",
    latestVitals: "Latest vitals",
    newConsultation: "New consultation (SOAP)",
    subjective: "Subjective",
    subjectivePlaceholder: "What the patient reports",
    examination: "Examination",
    examinationPlaceholder: "Objective exam findings",
    assessment: "Assessment",
    assessmentPlaceholder: "Clinical impression",
    plan: "Plan",
    planPlaceholder: "Tests, treatment, follow-up",
    saveConsultation: "Save consultation",
    addDiagnosis: "Add diagnosis",
    icd10Code: "ICD-10 code",
    icd10Placeholder: "e.g. J18.9",
    description: "Description",
    diagnosisDescPlaceholder: "Diagnosis description",
    primaryDiagnosis: "Primary diagnosis",
    ordersResults: "Orders & results",
    abnormal: "Abnormal",
    refRange: "(ref {range})",
    noOrders: "No tests ordered on this visit yet.",
    testType: "Test type",
    deleteOrder: "Delete test",
    deletePrescription: "Delete medication",
    confirmDelete: "Delete?",
    cancelDelete: "Cancel",
    test: "Test",
    testPlaceholder: "e.g. Full Blood Count",
    orderTest: "Order test",
    prescriptions: "Prescriptions",
    drugAllergiesLabel: "Drug allergies:",
    reviewBeforePrescribing: "Review before prescribing.",
    allergiesNotAssessedRx: "Allergies not assessed — confirm before prescribing.",
    noPrescriptions: "Nothing prescribed on this visit yet.",
    marLog: "Administration log",
    drug: "Drug",
    drugPlaceholder: "e.g. Amoxicillin-clavulanate",
    dose: "Dose",
    dosePlaceholder: "e.g. 625 mg",
    route: "Route",
    routePlaceholder: "e.g. oral",
    frequency: "Frequency",
    frequencyPlaceholder: "e.g. every 8 hours",
    duration: "Duration",
    durationPlaceholder: "e.g. 5 days",
    instructions: "Instructions",
    instructionsPlaceholder: "Optional — e.g. take with meals",
    mealTimingPlaceholder: "Meal timing",
    prescribe: "Prescribe",
    disposition: "Disposition",
    dispositionHint:
      "Decide where the patient goes next. Admit opens an inpatient stay; the choice is logged to the treatment record.",
    dispositionDischargeHome: "Discharge home",
    dispositionAdmit: "Admit",
    dispositionObservation: "Observation",
    dispositionRefer: "Refer",
    // Admit dialog
    admitDialogTitle: "Admit patient",
    admitDialogDesc: "Choose where to admit {name}. Beds shown are free.",
    department: "Department",
    anyDepartment: "Any department",
    selectDepartment: "Select a department",
    selectWard: "Select a ward",
    noWardsForDept: "No active wards in this department.",
    noFreeBeds: "No free beds in this ward.",
    admitReason: "Reason for admission",
    admitReasonPlaceholder: "e.g. Observation for chest pain, IV antibiotics",
    admitBedRequired: "Select a bed to admit the patient.",
    confirmAdmit: "Admit patient",
    // Observation dialog
    obsDialogTitle: "Place under observation",
    obsDialogDesc: "Set up observation for {name}.",
    obsReason: "What are you observing for?",
    obsReasonPlaceholder: "e.g. Post-concussion neuro observation",
    obsReasonRequired: "Enter what the patient is being observed for.",
    obsDuration: "Duration",
    obsDurationPlaceholder: "How long?",
    obsLocation: "Location",
    obsLocationPlaceholder: "e.g. ED observation bay 3",
    obsDur1h: "1 hour",
    obsDur2h: "2 hours",
    obsDur4h: "4 hours",
    obsDur6h: "6 hours",
    obsDur12h: "12 hours",
    obsDur24h: "24 hours",
    obsDur48h: "48 hours",
    obsDur72h: "72 hours",
    confirmObservation: "Start observation",
    // Referral dialog
    referDialogTitle: "Refer patient",
    referDialogDesc: "Refer {name} to another facility or specialist.",
    referFacility: "Refer to (facility / department)",
    referFacilityPlaceholder: "e.g. Central Hospital — Cardiology",
    referRecipient: "Recipient (person, if known)",
    referRecipientPlaceholder: "e.g. Dr. Amina Diallo",
    referReason: "Reason for referral",
    referReasonPlaceholder: "e.g. Needs cardiology review for arrhythmia",
    referRequired: "Enter both a facility and a reason to refer.",
    confirmReferral: "Record referral",
    clearanceGates: "Clearance gates",
    clearanceMedical: "Medical cleared",
    clearanceFinancial: "Financial cleared",
    clearancePharmacy: "Pharmacy ready",
    placementTransfers: "Placement & transfers",
    ward: "Ward",
    bed: "Bed",
    unassigned: "Unassigned",
    noBed: "No bed",
    selectBed: "Select a bed",
    attendingDoctor: "Attending doctor",
    selectDoctor: "Select a doctor",
    reason: "Reason",
    reasonPlaceholder: "Optional — e.g. Step-down from ICU",
    recordTransfer: "Record transfer",
    assignBed: "Assign bed",
    transferHistory: "Transfer history",
    bedMove: "Bed {from} → {to}",
    doctorMove: "Doctor {from} → {to}",
    byStaff: "by {name}",
    transferNoChange: "Choose a different bed or doctor to record a move.",
    transferFailed: "Could not record the transfer.",
    transferDoneBed: "Moved to {placement}.",
    transferDoneNoBed: "Bed released.",
    transferDoneDoctor: "Now under {doctor}.",
    transferDoneNoDoctor: "Attending doctor cleared.",
    careStage: "Care stage",
    journeyComplete: "Care journey complete.",
    dischargeBlocked: "Discharge blocked",
    blockerMedical: "Medical clearance pending",
    blockerFinancial: "Financial clearance pending",
    blockerPharmacy: "Pharmacy not ready",
    blockerReconcile: "Anonymous emergency profile must be reconciled first",
    dischargeFollowUp: "Discharge & follow up",
    dischargeConfirm: "Confirm discharge",
    dischargeConfirmBody:
      "This closes {name}'s visit and moves them off the board. Their record stays on file.",
    advanceTo: "Advance to {stage}",
    deceasedRecorded: "Recorded as deceased.",
    deceasedRecordedOn: "Recorded as deceased on {date}.",
    recordDeath: "Record death",
    recordDeathConfirmTitle: "Record death",
    recordDeathConfirmBody:
      "This records {name} as deceased and closes their visit. This is a terminal outcome and skips the discharge clearance checks. Their record stays on file.",
    recordDeathNote: "Note (optional)",
    recordDeathNotePlaceholder: "Circumstances or cause, if known",
    recordDeathConfirm: "Confirm",
    logVitals: "Log vitals & assessment",
    vitalsSpo2: "SpO₂ (%)",
    vitalsPulse: "Pulse (bpm)",
    vitalsSys: "BP systolic",
    vitalsDia: "BP diastolic",
    vitalsTemp: "Temp (°C)",
    vitalsWeight: "Weight (kg)",
    vitalsGcs: "GCS (3–15)",
    vitalsInvalid: "Couldn't save — check the vitals values and try again.",
    vitalsInvalidFields: "Couldn't save — check {fields} (value out of range).",
    notes: "Notes",
    notesPlaceholder: "Clinical observations",
    saveLog: "Save log entry",
    treatmentHistory: "Treatment history",
    noEntries: "No entries logged yet",
    vitalsOutOfRange:
      "{field} is out of the plausible range ({min}–{max}). Entry not saved — please check the value.",
    historyTitle: "Visit history",
    historyEmpty: "No visits on record for this patient.",
    historyCurrent: "Current",
    emptyNote: "Empty note.",
    showMore: "More options",
    showLess: "Fewer options",
  },

  reports: {
    unassigned: "Unassigned",
    unknown: "Unknown department",
    loading: "Loading reports…",
    title: "Operations & analytics",
    subtitle:
      "Hospital-wide activity, occupancy, clinical and pharmacy metrics for the selected period. Export a full snapshot as PDF or Excel.",
    pdf: "PDF",
    excel: "Excel",
    partialNotice:
      "This period reaches beyond the records kept on this device. Connect to the internet to include the full history — figures shown may be incomplete.",
    startDate: "Start date",
    endDate: "End date",
    to: "to",
    exportsReflect: "Exports reflect this period",
    resultsRecordedOne: "{count} result recorded in period.",
    resultsRecordedOther: "{count} results recorded in period.",
    abnormalNormal: "{abnormal} abnormal · {normal} normal",
    noResults: "No results recorded in this period.",
    noData: "No data in this period",
    seriesCount: "Count",
    exportTitle: "CareFlow — Hospital Operations Report",
    reportingPeriod: "Reporting period",
    generated: "Generated",
    allTime: "All time",
    metric: "Metric",
    value: "Value",
    pageOf: "Page {p} of {total}",
    visitsByType: "Visits by type",
    kpi: {
      totalVisits: "Total visits",
      uniquePatients: "Unique patients",
      admissions: "Admissions",
      admissionsStarted: "Admissions started",
      discharges: "Discharges",
      deaths: "Deaths",
      bedOccupancy: "Bed occupancy",
      outpatient: "Outpatient",
      inpatient: "Inpatient",
      emergency: "Emergency",
      currentInpatients: "Current inpatients",
      avgLos: "Avg length of stay",
      medianLos: "Median length of stay (days)",
    },
    chart: {
      visitVolume: "Visit volume over time",
      visitVolumeDesc:
        "Daily (or weekly for long spans) arrivals, stacked by visit type.",
      visitTypeMix: "Visit type mix",
      departmentThroughput: "Department throughput",
      topDiagnoses: "Top diagnoses",
      lengthOfStay: "Length of stay",
      lengthOfStayDesc: "Discharged admissions in period.",
      wardOccupancy: "Ward occupancy",
      openByStage: "Open visits by care stage",
      outcomes: "Visit outcomes",
      outcomesDesc: "Closed visits in period, by terminal outcome.",
      patientsBySex: "Patients by sex",
      patientsByAge: "Patients by age",
      clearanceBottlenecks: "Discharge clearance bottlenecks",
      clearanceBottlenecksDesc: "Active admissions still pending each gate.",
      diagnosticResults: "Diagnostic results",
    },
    table: {
      period: "Period",
      outpatient: "Outpatient",
      inpatient: "Inpatient",
      emergency: "Emergency",
      total: "Total",
      band: "Band",
      discharges: "Discharges",
      ward: "Ward",
      totalBeds: "Total beds",
      occupied: "Occupied",
      free: "Free",
      occupancyPct: "Occupancy %",
      stage: "Stage",
      visits: "Visits",
      outcome: "Outcome",
      count: "Count",
      abnormal: "Abnormal",
      normal: "Normal",
      abnormalRate: "Abnormal rate",
      sex: "Sex",
      patients: "Patients",
      ageBand: "Age band",
      gate: "Gate",
      pending: "Pending",
      category: "Category",
      cases: "Cases",
    },
    sheet: {
      summary: "Summary",
      visitsOverTime: "Visits over time",
      visitTypes: "Visit types",
      departments: "Departments",
      topDiagnoses: "Top diagnoses",
      lengthOfStay: "Length of stay",
      wardOccupancy: "Ward occupancy",
      careStages: "Care stages",
      outcomes: "Visit outcomes",
      sexMix: "Sex mix",
      ageMix: "Age mix",
      clearanceGates: "Clearance gates",
    },
    los: {
      lt1: "< 1 day",
      d1_2: "1–2 days",
      d3_4: "3–4 days",
      d5_7: "5–7 days",
      d8_14: "8–14 days",
      d15: "15+ days",
    },
    clearance: {
      medical: "Medical clearance",
      financial: "Financial clearance",
      pharmacy: "Pharmacy ready",
    },
    view: {
      charts: "Analytics",
      register: "Patient register",
    },
    register: {
      exportTitle: "CareFlow — Patient Register",
      count: "{count} patients in period",
      csv: "CSV",
      sheet: "Patient register",
      empty: "No patients match these filters for this period.",
      searchPlaceholder: "Search name or ID",
      filterStatus: "Status",
      filterType: "Visit type",
      filterDepartment: "Department",
      filterSex: "Sex",
      allTypes: "All types",
      allDepartments: "All departments",
      allSexes: "All sexes",
      statusFilter: {
        all: "All patients",
        in_hospital: "Currently in hospital",
        open: "Open visits",
        discharged: "Discharged",
      },
      statusValue: {
        open: "Open",
        closed: "Closed",
        cancelled: "Cancelled",
      },
      case: {
        new: "New",
        old: "Returning",
      },
      outcome: {
        inCare: "In care",
        discharged: "Discharged",
        deceased: "Deceased",
        cancelled: "Cancelled",
      },
      ageYears: "{n} yr",
      ageMonths: "{n} mo",
      col: {
        no: "#",
        date: "Date",
        case: "Case",
        patientCode: "Patient ID",
        name: "Names",
        sex: "Sex",
        age: "Age",
        phone: "Contact",
        residence: "Residence",
        type: "Type",
        department: "Department",
        weight: "Weight (kg)",
        bp: "BP",
        temp: "Temp (°C)",
        chiefComplaint: "Chief complaint",
        entryDiagnosis: "Entry diagnosis",
        investigations: "Investigations",
        treatment: "Treatment",
        allergies: "Past history / allergies",
        doctor: "Attending doctor",
        admitDate: "Date of entry",
        dischargeDate: "Date of discharge",
        los: "Days",
        outcome: "Outcome",
        status: "Visit status",
      },
    },
  },

  staff: {
    title: "Staff Directory",
    members: "members",
    subtitle: "Clinical and administrative staff, grouped by role.",
    loading: "Loading directory…",
    editTitle: "Edit staff member",
    editDesc: "Update this team member's directory details.",
    saving: "Saving…",
    updateFailed: "Could not update the staff member.",
    deactivate: "Deactivate",
    reactivate: "Reactivate",
    deactivatePrompt: "Deactivate this staff member?",
    deactivateConfirm: "Deactivate",
    resetPassword: "Reset password",
    resetPasswordTitle: "Reset password",
    resetPasswordDesc:
      "Set a new sign-in password for {name}, then hand it to them directly.",
    newPassword: "New password",
    newPasswordPlaceholder: "At least 8 characters",
    resetPasswordTooShort: "Password must be at least 8 characters.",
    resetPasswordSuccess: "Password updated.",
    resetNoSession: "Your session has expired — please sign in again.",
    resetting: "Updating…",
    resetConfirm: "Set password",
    groupDoctors: "Doctors",
    groupNursing: "Nursing",
    groupLaboratory: "Laboratory",
    groupPharmacy: "Pharmacy",
    groupFrontDesk: "Front Desk",
    groupAdministration: "Administration",
    active: "Active",
    inactive: "Inactive",
    attending: "Attending · {count}",
    noActivePatients: "No active patients",
    unknownPatient: "Unknown patient",
    // Add-staff form
    newStaff: "Add staff member",
    newTitle: "Add staff member",
    newDesc: "Register a clinical or administrative team member for your hospital.",
    name: "Full name",
    namePlaceholder: "e.g. Dr. Amina Diallo",
    role: "Role",
    rolePlaceholder: "Select a role",
    department: "Department",
    departmentNone: "No department",
    username: "Username",
    usernamePlaceholder: "e.g. adiallo",
    password: "Password",
    passwordPlaceholder: "At least 6 characters",
    email: "Email",
    emailPlaceholder: "name@hospital.cm",
    phone: "Phone",
    phonePlaceholder: "+237…",
    invalidEmail: "Enter a valid email address.",
    invalidPhone: "Enter a valid phone number for the selected country.",
    create: "Add member",
    creating: "Creating…",
    nameRequired: "A full name is required.",
    roleRequired: "Please select a role.",
    usernameRequired: "A username is required.",
    passwordTooShort: "Password must be at least 6 characters.",
    noHospital: "No active hospital — please sign in again.",
  },

  reconciliation: {
    title: "Profile Reconciliation",
    pending: "pending",
    subtitle:
      "Give an unidentified emergency record its real identity. Clinical logs are preserved on the same record.",
    loading: "Loading worklist…",
    allReconciled: "All records reconciled",
    allReconciledHint: "No unidentified emergency profiles are awaiting a match.",
    unidentified: "Unidentified",
    logsOne: "{count} log",
    logsOther: "{count} logs",
    arrived: "Arrived {time}",
    justNow: "just now",
    hoursAgo: "{count}h ago",
    daysAgo: "{count}d ago",
    mergeInto: "Merge into",
    selectVerifiedPatient: "Select verified patient",
    merge: "Merge",
    // Worklist search + per-record action
    searchPlaceholder: "Search by tag, ID or reason…",
    noMatches: "No emergency records match your search.",
    reconcile: "Reconcile",
    // Reconcile dialog
    dialogTitle: "Reconcile emergency record",
    dialogSubtitle:
      "Enter the patient's real details to give this emergency record a permanent identity.",
    detailsHeading: "Patient details",
    existingHeading: "Already in the system?",
    existingHint:
      "If this patient already has a record, link it to merge instead of creating a new identity.",
    searchExisting: "Search existing patients…",
    linkedTo: "Linking to",
    unlink: "Use a new identity instead",
    matchFound: "{count} match",
    matchesFound: "{count} matches",
    noExistingMatches: "No existing patients found.",
    mergeWarning:
      "This record's visits and history will be merged into the selected patient, and this placeholder removed.",
    confirmReconcile: "Confirm & reconcile",
    confirmMerge: "Confirm & merge",
    cancel: "Cancel",
    successReconciled: "{name} reconciled — patient ID {id}.",
    successMerged: "Merged into {name}.",
  },

  departments: {
    title: "Departments",
    total: "total",
    subtitle: "Manage the clinical and administrative units patients are routed to.",
    newDepartment: "New department",
    archiveTitle: "Archive department",
    archiveBody:
      "Archive “{name}”? It will be hidden from routing and board filters. You can reactivate it at any time.",
    archiveConfirm: "Archive",
    updateFailed: "Could not update the department.",
    unroutedOne: "active visit not yet routed to a department.",
    unroutedOther: "active visits not yet routed to a department.",
    loading: "Loading departments…",
    active: "Active",
    archived: "Archived",
    activeVisitOne: "active visit",
    activeVisitOther: "active visits",
    edit: "Edit",
    newTitle: "New department",
    editTitle: "Edit department",
    newDesc: "Create a unit patients can be routed to at registration.",
    editDesc: "Update this unit's details or archive it.",
    name: "Name",
    namePlaceholder: "e.g. Maternity",
    code: "Code",
    codePlaceholder: "Optional — e.g. MAT",
    description: "Description",
    descriptionPlaceholder: "Optional — what this unit handles",
    activeHint: "Archived units are hidden from routing & board filters.",
    nameRequired: "A department name is required.",
    create: "Create department",
  },

  diagnostics: {
    title: "Diagnostics queue",
    awaiting: "awaiting result",
    subtitle:
      "Tests ordered by clinicians, awaiting a recorded result. Enter a result to close the loop and return it to the patient's record.",
    loading: "Loading queue…",
    queueClear: "Queue clear",
    queueClearHint: "No outstanding lab, imaging or procedure orders.",
    start: "Start",
    enterResult: "Enter result",
    unknownPatient: "Unknown patient",
    recordResult: "Record result",
    resultValue: "Result value",
    resultValuePlaceholder: "e.g. 7.8% · Normal · 0.04 ng/mL",
    referenceRange: "Reference range",
    referenceRangePlaceholder: "Optional — e.g. < 7.0%",
    summary: "Summary / interpretation",
    summaryPlaceholder: "Optional — narrative findings for the clinician",
    attachment: "Attachment",
    attachmentHint:
      "Optional — attach a scan or PDF (image/PDF). Uploaded to secure, hospital-private storage.",
    attachmentUploading: "Uploading…",
    attachmentFailed: "Upload failed. Please try again.",
    attachmentNoHospital: "No active hospital — cannot upload.",
    attachmentView: "View attachment",
    attachmentViewFailed: "Couldn't open — retry",
    abnormalResult: "Abnormal result",
    abnormalHint: "Flags this result for clinician review.",
  },

  meds: {
    title: "Medication round",
    overdueDueNow: "{overdue} overdue · {due} due now",
    subtitle:
      "Doses due across the floor. Record each one as given, held or refused — it stamps the time and your name so the next shift knows exactly what was done.",
    shiftHandover: "Shift handover · outstanding by ward",
    active: "{count} active",
    overdue: "{count} overdue",
    due: "{count} due",
    clear: "clear",
    loading: "Loading medications…",
    noActive: "No active medications",
    noActiveHint: "No active prescriptions on the floor right now.",
    inpatientByWard: "Inpatient · by ward",
    outpatientEd: "Outpatient & ED",
    duePrefix: "due ",
    last: "Last: {status}",
    lastWithTime: "Last: {status} · {time}",
    agoSpan: "{span} ago",
    inSpan: "in {span}",
    reasonTitle: "Reason — {status}",
    reasonDesc: "Document why this dose was not given. The doctor will see it.",
    reasonLabel: "Reason",
    reasonPlaceholder: "e.g. patient NPO for surgery; BP too low; refused",
    reasonConfirm: "Save",
    reasonCancel: "Cancel",
    unassigned: "Unassigned",
    unknownPatient: "Unknown patient",
  },

  intake: {
    title: "Patient Intake",
    subtitle: "Open a new visit on the Live Status Board.",
    visitOpened: "Visit opened",
    visitOpenedHint: "Added to the Live Board in the Intake stage.",
    emergencyTag: "Emergency tracking tag",
    patient: "Patient",
    viewOnBoard: "View on board",
    registerAnother: "Register another",
    emergencyToggle: "Emergency Unconscious Intake",
    emergencyToggleHint:
      "Skip identity paperwork and assign an anonymous tracking tag.",
    emergencyRecord: "Emergency record",
    patientDetails: "Patient details",
    emergencyFieldsHidden: "Personal fields are hidden. A tracking tag (e.g.",
    emergencyFieldsHiddenSuffix:
      ") is generated on submit; the patient ID is assigned once their identity is reconciled.",
    fullName: "Full name",
    fullNamePlaceholder: "e.g. Ada Mensah",
    sex: "Sex",
    dob: "Date of birth",
    dobUnknown: "Don't know the exact date? Enter age instead",
    useExactDob: "Enter exact date of birth instead",
    dobFutureError: "Date of birth cannot be in the future.",
    approxAge: "Approximate age (years)",
    approxAgePlaceholder: "e.g. 45",
    phone: "Phone",
    phonePlaceholder: "Optional",
    nationalId: "National ID",
    nationalIdPlaceholder: "Optional",
    motherFirstName: "Mother's first name",
    motherFirstNamePlaceholder: "Optional — e.g. Ndung",
    motherFirstNameHint:
      "Used to build the patient ID's trailing initial. Can be added later.",
    occupation: "Occupation",
    occupationPlaceholder: "Optional — e.g. Market trader",
    maritalStatus: "Marital status",
    emergencyContactName: "Emergency contact",
    emergencyContactNamePlaceholder: "Optional — name",
    emergencyContactPhone: "Emergency contact phone",
    background: "Background (optional)",
    backgroundShow: "Show",
    backgroundHide: "Hide",
    backgroundHint:
      "Quick-add known history (past illnesses, family, social). The doctor can complete the rest in consultation.",
    backgroundPlaceholder: "e.g. Type 2 diabetes",
    patientIdTag: "Patient ID",
    visit: "Visit",
    reason: "Presenting reason / chief complaint",
    reasonPlaceholder: "e.g. Head trauma following road traffic accident",
    triage: "Triage priority",
    triageNone: "Not triaged yet",
    department: "Department",
    selectDepartment: "Select department",
    registeringStaff: "Registering staff",
    selectStaff: "Select staff",
    attendingDoctor: "Attending doctor",
    unassigned: "Unassigned",
    selectStaffError: "Select the registering staff member.",
    reasonRequired: "A presenting reason is required.",
    nameRequired: "Patient name is required for a standard intake.",
    invalidPhone: "Enter a valid phone number for the selected country.",
    registerEmergency: "Register emergency patient",
    register: "Register patient",
  },

  floorMap: {
    title: "Floor map",
    wardOne: "ward",
    wardOther: "wards",
    subtitle:
      "Live bed occupancy by floor and ward. Add wards and beds, set bed status, and see who occupies each bed.",
    newWard: "New ward",
    loading: "Loading floor map…",
    noWards: "No wards yet. Create your first ward to start mapping beds.",
    occupied: "Occupied",
    free: "Free",
    other: "Other",
    edit: "Edit",
    noBeds: "No beds in this ward yet — edit the ward to add some.",
    newTitle: "New ward",
    editTitle: "Edit ward",
    newDesc: "Create a ward and optionally pre-fill it with beds.",
    editDesc: "Update ward details, add beds, or change bed status.",
    name: "Name",
    namePlaceholder: "e.g. Medical Ward B",
    block: "Block",
    blockPlaceholder: "Optional — e.g. Block A",
    floor: "Floor",
    floorPlaceholder: "Optional — e.g. 2nd Floor",
    department: "Department",
    noDepartment: "No department",
    initialBeds: "Initial beds",
    initialBedsHint: "Beds are labelled “Bed 1”, “Bed 2”, … and start free.",
    beds: "Beds",
    addCountLabel: "Number of beds to add",
    add: "Add",
    noBedsYet: "No beds yet. Add some above.",
    removeBed: "Remove {label}",
    nameRequired: "A ward name is required.",
    enterBedCount: "Enter how many beds to add.",
    couldNotUpdate: "Could not update the bed.",
    couldNotRemove: "Could not remove the bed.",
    cancel: "Cancel",
    done: "Done",
    create: "Create ward",
    saveWard: "Save ward",
  },
  search: {
    trigger: "Find a patient",
    placeholder: "Search by name or hospital number",
    title: "Find a patient",
    description: "Search by name, hospital number, phone, or national ID.",
    typeToSearch: "Type a name or hospital number to search.",
    noResults: "No patients match “{query}”.",
    hospitalNo: "Hospital no.",
    noVisit: "No visit on record",
    open: "Open",
    openVisit: "Open patient",
    anonymous: "Unidentified — emergency intake",
    resultsCount: "{count} match",
    resultsCountPlural: "{count} matches",
  },
  demo: {
    title: "Demo controls",
    resetTitle: "Reset demo data",
    resetBody:
      "Restore the clean sample hospital. This discards any changes made on this device.",
    resetButton: "Reset",
    resetConfirm: "Reset everything",
  },
  sync: {
    online: "Online — every change is saved.",
    offlineSaved: "Saved on this device. Will sync when the internet returns.",
    offlinePendingOne:
      "Saved on this device. 1 change will sync when the internet returns.",
    offlinePendingMany:
      "Saved on this device. {count} changes will sync when the internet returns.",
    queuedOne: "Saving 1 change…",
    queuedMany: "Saving {count} changes…",
    panelTitle: "Sync health",
    panelIntro:
      "Every change is saved on this device first, then sent to the hospital's records when the internet allows.",
    allSynced: "Everything is saved and synced.",
    groupWaiting: "Waiting to send",
    groupRetrying: "Retrying",
    groupAttention: "Needs attention",
    groupAttentionHint:
      "These changes keep failing to send. Show this to your administrator — the details below say why.",
    entryQueuedAt: "queued {time}",
    entryAttempts: "{count} failed attempts",
    opInsert: "New record",
    opUpdate: "Change",
    opDelete: "Deletion",
    syncNow: "Sync now",
    syncing: "Syncing…",
    offlineHint: "You're offline. Changes will send when the internet returns.",
    groupConflicts: "Didn't save — someone else changed it first",
    groupConflictsHint:
      "A colleague changed the same record before your edit reached the server. Your edit was kept here — review and re-apply it, or discard it if theirs should stand.",
    conflictDetectedAt: "detected {time}",
    reapply: "Re-apply my change",
    retry: "Retry",
    discard: "Discard",
  },
  notify: {
    regionLabel: "Notifications",
    dismiss: "Dismiss",
    storageFullTitle: "This device's storage is full",
    storageFullBody:
      "Your latest change could not be saved on this device. Free up space or contact your administrator — until then, new work may be lost.",
    storageWriteFailedTitle: "Could not save on this device",
    storageWriteFailedBody:
      "Something prevented saving your latest change to this device. Try again; if it keeps happening, contact your administrator.",
    cacheResetTitle: "Local data was reset",
    cacheResetBody:
      "The copy of the records stored on this device was unreadable and has been replaced with a fresh copy from the server.",
    outboxWriteFailedTitle: "Could not queue your change for sync",
    outboxWriteFailedBody:
      "Your change was saved on this device but could not join the sync queue. It may not reach the hospital's records — contact your administrator.",
    outboxCorruptTitle: "The sync queue was unreadable",
    outboxCorruptBody:
      "Changes waiting to sync could not be read and were set aside (a backup copy was kept on this device). Recent unsent work may need to be re-entered — contact your administrator.",
    conflictTitle: "A change didn't save — someone else edited first",
    conflictBody:
      "A colleague changed the same record before your edit reached the server. Open the sync panel (cloud icon, top bar) to review and re-apply your change.",
    uploadQueuedTitle: "File saved — will upload when online",
    uploadQueuedBody:
      "The result was recorded. Its attached file is saved on this device and will upload automatically when the internet returns.",
    uploadRetryTitle: "A file is waiting to upload",
    uploadRetryBody:
      "An attached file couldn't be uploaded yet. It stays saved on this device and will upload automatically when the connection allows.",
    signOutPendingTitle: "Some changes haven't synced yet",
    signOutPendingBody:
      "You're signed out, but changes made on this device couldn't be sent to the hospital's records yet. They stay saved here and will send after the next sign-in on this device.",
  },
  tableNames: {
    hospitals: "Hospital details",
    departments: "Department",
    wards: "Ward",
    beds: "Bed",
    staff: "Staff member",
    patients: "Patient record",
    allergies: "Allergy",
    visits: "Visit",
    consultations: "Consultation",
    diagnoses: "Diagnosis",
    orders: "Test order",
    results: "Test result",
    prescriptions: "Prescription",
    medication_administrations: "Medication given",
    treatment_records: "Treatment note",
    admissions: "Admission",
    transfers: "Transfer",
    care_plan_items: "Care plan task",
    care_plan_entries: "Care plan note",
    billable_items: "Price list item",
    charges: "Billing charge",
    clinical_terms: "Medical term",
    follow_up_tasks: "Follow-up task",
    patient_history: "Background record",
    ros_responses: "ROS answer",
    notifications: "Notification",
  },
  visitReport: {
    button: "Patient report",
    buttonLong: "Download patient report",
    title: "Patient Visit Report",
    generated: "Generated",
    disclaimer:
      "Generated by CareFlow from this hospital's records. Give to the patient or share with another facility.",
    section: {
      patient: "Patient",
      visit: "Visit",
      allergies: "Allergies",
      background: "Clinical background",
      vitals: "Vital signs",
      doctorNotes: "Doctor's notes",
      diagnoses: "Diagnoses",
      tests: "Tests & results",
      medications: "Medications",
      admission: "Admission",
    },
    field: {
      name: "Name",
      hospitalNo: "Patient ID",
      dob: "Date of birth",
      age: "Age",
      sex: "Sex",
      phone: "Phone",
      address: "Address",
      nationalId: "National ID",
      occupation: "Occupation",
      maritalStatus: "Marital status",
      emergencyContact: "Emergency contact",
      detail: "Detail",
      since: "Since",
      ros: "Review of systems",
      visitType: "Visit type",
      department: "Department",
      attendingDoctor: "Attending doctor",
      registeredBy: "Registered by",
      arrived: "Arrived",
      closed: "Closed",
      careStage: "Care stage",
      triage: "Triage priority",
      chiefComplaint: "Reason for visit",
      triageNotes: "Triage notes",
      substance: "Substance",
      category: "Category",
      severity: "Severity",
      reaction: "Reaction",
      recordedAt: "Recorded",
      pulse: "Pulse",
      temp: "Temp",
      notes: "Notes",
      subjective: "Patient's account",
      examination: "Examination findings",
      assessment: "Assessment",
      plan: "Plan",
      diagnosis: "Diagnosis",
      icd10: "ICD-10",
      primary: "Primary",
      diagnosedBy: "Diagnosed by",
      test: "Test",
      type: "Type",
      status: "Status",
      result: "Result",
      flag: "Flag",
      drug: "Medication",
      dose: "Dose",
      route: "Route",
      frequency: "Frequency",
      duration: "Duration",
      administered: "Doses given",
      ward: "Ward",
      bed: "Bed",
      admitted: "Admitted",
      discharged: "Discharged",
      lengthOfStay: "Length of stay",
      reason: "Reason",
      transfers: "Transfers",
      date: "Date",
      fromTo: "Movement",
    },
    value: {
      years: "{count} years",
      ongoing: "Ongoing",
      yes: "Yes",
      abnormal: "Abnormal",
      normal: "Normal",
      stillAdmitted: "Still admitted",
      days: "{count} days",
    },
    empty: {
      noKnownAllergies: "No known allergies.",
      allergiesNotAssessed: "Allergies not yet assessed.",
      noBackground: "No clinical background recorded.",
      noVitals: "No vital signs recorded.",
      noNotes: "No doctor's notes recorded.",
      noDiagnoses: "No diagnoses recorded.",
      noTests: "No tests ordered.",
      noMedications: "No medications prescribed.",
    },
    history: {
      button: "Full history",
      buttonLong: "Download full history",
      title: "Patient Medical History",
      overview: "History overview",
      totalVisits: "Total visits",
      firstVisit: "First visit",
      lastVisit: "Most recent visit",
      visitN: "Visit {n} of {total}",
      noVisits: "No visits on record.",
    },
  },

  carePlan: {
    title: "Care plans",
    subtitle:
      "Individualized nursing care for admitted patients — the daily needs that aren't on a drug chart. Record what was done, set the goals, and hand over cleanly to the next shift.",
    loading: "Loading care plans…",
    selectPatient: "Select a patient",
    selectPatientHint: "Pick an admitted patient to see and update their care plan.",
    noPatients: "No admitted patients",
    noPatientsHint: "Care plans cover inpatients. No one is admitted right now.",
    allWards: "All wards",
    wardFilter: "Ward",
    needsActive: "{count} active",
    needsNone: "No active needs",
    handoverWaiting: "Handover waiting",
    needsBlock: "Care needs",
    needsBlockHint: "What this patient needs help with, and the goal for each.",
    noNeeds: "No care needs recorded yet.",
    addNeed: "Add care need",
    category: {
      breathing: "Breathing",
      nutrition: "Eating & drinking",
      elimination: "Elimination",
      mobility_positioning: "Mobility & positioning",
      sleep_rest: "Sleep & rest",
      hygiene: "Hygiene",
      temperature: "Body temperature",
      dressing: "Dressing & grooming",
      safety: "Safety",
      communication_emotional: "Communication & emotional",
      pain_comfort: "Pain & comfort",
      spiritual: "Spiritual & beliefs",
      wound_skin_care: "Wound & skin care",
      other: "Other",
    },
    field: {
      category: "Need",
      description: "What's needed",
      descriptionPlaceholder: "e.g. Assist with full bed bath each morning",
      frequency: "How often",
      frequencyPlaceholder: "e.g. Twice daily, every 2 hours",
      goal: "Goal",
      goalPlaceholder: "e.g. Skin intact, no pressure areas",
    },
    statusActive: "Active",
    statusResolved: "Resolved",
    resolve: "Mark resolved",
    resolvedOn: "Resolved",
    saveNeed: "Save care need",
    logBlock: "Care log",
    logBlockHint: "Append-only record of care given and observations. Entries can't be edited.",
    noLog: "No care recorded yet.",
    recordCare: "Record care",
    handoverBlock: "Shift handover",
    handoverBlockHint: "A note for the next shift. The latest handover shows at the top of the plan.",
    latestHandover: "Latest handover",
    noHandover: "No handover left yet.",
    leaveHandover: "Leave handover",
    noteField: "Note",
    notePlaceholder: "e.g. Tolerated soft diet well, ambulated to chair x2",
    handoverNotePlaceholder: "e.g. Watch left heel — early redness. Repositioning 2-hourly.",
    markHandover: "Mark as shift handover",
    saveNote: "Save note",
    handoverTag: "Handover",
    forNeed: "Re: {need}",
    generalNote: "General",
    // Phase 20 — shared doctor↔nurse care orders
    ordersBlock: "Care & orders",
    kindInstruction: "Doctor instruction",
    kindMonitoring: "Monitoring",
    kindNursing: "Nursing care",
    addInstruction: "Add instruction",
    addInstructionPlaceholder: "e.g. Encourage oral fluids, aim 2 L/day",
    addMonitoring: "Add monitoring",
    addMonitoringPlaceholder: "e.g. Vitals + GCS",
    markDone: "Done",
    flagForDoctor: "Flag for doctor",
    flagPlaceholder: "Something the doctor should see…",
    flagged: "Flag for doctor",
    acknowledge: "Acknowledge",
    awaitingDoctor: "Awaiting doctor",
    raisedBy: "Raised by {name}",
    vitalsTrend: "Vitals trend",
    vTime: "Time",
    vPulse: "Pulse",
    vTemp: "Temp",
    dueBadge: "{n} due",
    needsYouBadge: "{n} needs you",
  },

  worklist: {
    title: "Worklist",
    subtitle:
      "Everything due, and everyone who needs a doctor — across every ward.",
    dueHeading: "Due now",
    needsYouHeading: "Needs a doctor",
    nothingDue: "Nothing due right now.",
    noneNeedYou: "No one needs a doctor right now.",
    loading: "Loading…",
    vitalsConcern: "Vitals need a look",
  },

  billing: {
    title: "Billing",
    subtitle:
      "Itemized bills per visit. Charges for tests, drugs, beds and nursing are generated automatically; add extras or a discount, then settle. Informational — it doesn't block discharge.",
    loading: "Loading billing…",
    // Patient/visit picker
    selectVisit: "Select a visit",
    selectVisitHint: "Pick a patient visit to see and build its bill.",
    searchPlaceholder: "Search by patient name or ID…",
    noVisits: "No visits found",
    noVisitsHint: "No visits match your search.",
    openVisits: "Open visits",
    closedVisits: "Closed visits",
    // Bill panel
    billFor: "Bill for {name}",
    visitMeta: "{type} · {date}",
    recalc: "Recalculate charges",
    recalcHint: "Re-derive automatic charges from the latest clinical record.",
    emptyBill: "No charges yet",
    emptyBillHint: "Recalculate to generate charges from this visit, or add a line manually.",
    // Table columns
    colItem: "Item",
    colQty: "Qty",
    colUnitPrice: "Unit price",
    colAmount: "Amount",
    colStatus: "Status",
    colActions: "",
    // Totals
    subtotal: "Subtotal",
    discountTotal: "Discounts",
    grandTotal: "Total due",
    settled: "Settled",
    // Category section headers
    category: {
      consultation: "Consultations",
      lab_test: "Lab tests",
      imaging: "Imaging",
      procedure: "Procedures",
      medication: "Medications",
      bed_per_night: "Beds",
      nursing_per_day: "Nursing",
      other: "Other",
    },
    // Charge status
    status: {
      pending: "Pending",
      paid: "Paid",
      waived: "Waived",
    },
    // Source labels
    source: {
      consultation: "Consultation",
      order: "Test",
      prescription: "Drug",
      bed: "Bed",
      nursing: "Nursing",
      procedure: "Procedure",
      manual: "Manual",
      discount: "Discount",
    },
    // Actions
    addCharge: "Add charge",
    addDiscount: "Add discount",
    settle: "Settle bill",
    settleConfirm: "Mark every pending charge as paid?",
    markPaid: "Mark paid",
    markWaived: "Waive",
    markPending: "Mark pending",
    remove: "Remove",
    exportPdf: "Export PDF",
    // PDF export
    pdfTitle: "Bill / Invoice",
    pdfDisclaimer:
      "Itemized statement of charges for this visit. Informational — not a tax receipt or proof of payment.",
    accessDenied: "Billing is available to administrators and receptionists only.",
    // Add-charge dialog
    addChargeTitle: "Add a charge",
    addDiscountTitle: "Add a discount",
    fromCatalog: "From price list",
    customLine: "Custom line",
    fieldItem: "Price-list item",
    fieldDescription: "Description",
    fieldDescriptionPlaceholder: "e.g. Wound dressing pack",
    fieldQuantity: "Quantity",
    fieldUnitPrice: "Unit price (XAF)",
    fieldDiscountAmount: "Discount amount (XAF)",
    discountDescriptionPlaceholder: "e.g. Goodwill discount",
    save: "Save",
    cancel: "Cancel",
    // Price-list (catalog) admin
    pricesTitle: "Price list",
    pricesSubtitle:
      "The hospital's billable services and goods, with the current price in XAF. Editing a price never changes bills already raised.",
    pricesLink: "Manage price list",
    backToBilling: "Back to billing",
    addItem: "Add item",
    editItem: "Edit item",
    newItem: "New price-list item",
    fieldName: "Name",
    fieldNamePlaceholder: "e.g. Consultation — General Medicine",
    fieldCategory: "Category",
    fieldUnit: "Unit",
    fieldRefCode: "Reference code",
    fieldRefCodePlaceholder: "Optional — links auto-charges",
    fieldActive: "Active",
    activeLabel: "Active",
    inactiveLabel: "Inactive",
    unit: {
      per_item: "Per item",
      per_night: "Per night",
      per_day: "Per day",
    },
    noItems: "No price-list items yet.",
    // Drawer summary
    drawerTitle: "Billing",
    drawerTotal: "Total due",
    drawerView: "Open full bill",
    drawerNoCharges: "No charges recorded.",
  },

  marketing: {
    signIn: "Sign in",
    getStarted: "Create your hospital account",
    heroEyebrow: "Hospital operations, digitized",
    heroTitle: "Run your whole hospital from one clear record.",
    heroSubtitle:
      "CareFlow tracks every patient from arrival to discharge, covering intake, consultation, tests, medication, beds and reports, so your team works from one shared, accurate record instead of scattered paper.",
    heroExplore: "See how it works",
    heroNote: "Built for clinics and hospitals in Cameroon. French and English.",

    // Mockup preview (decorative product UI in the hero / showcases)
    mockBoardTitle: "Live board",
    mockBoardWaiting: "waiting",
    mockReportsTitle: "This week",
    mockReportsAdmissions: "Admissions",
    mockReportsOutpatients: "Outpatients",
    mockReportsOccupancy: "Bed occupancy",

    // Trust / outcome strip
    statRecordValue: "1",
    statRecordLabel: "Shared record across every department",
    statRolesValue: "6",
    statRolesLabel: "Clinical roles, one workflow",
    statBilingualValue: "FR · EN",
    statBilingualLabel: "Fully bilingual interface",
    statOfflineValue: "Offline",
    statOfflineLabel: "Keeps working when the internet drops",

    // Patient journey
    journeyEyebrow: "The patient journey",
    journeyTitle: "Every step, handed cleanly to the next",
    journeySubtitle:
      "CareFlow mirrors how your floor already works, so nothing is lost between the registration desk, the consulting room and the ward.",
    journeyStep1Title: "Intake & triage",
    journeyStep1Body:
      "Register a patient in seconds, capture vitals, and set acuity so the sickest are seen first.",
    journeyStep2Title: "Consultation",
    journeyStep2Body:
      "The doctor records history, examination, assessment and plan against the full picture.",
    journeyStep3Title: "Tests & results",
    journeyStep3Body:
      "Order labs and imaging, then attach the results straight onto the visit.",
    journeyStep4Title: "Treatment & beds",
    journeyStep4Body:
      "Prescribe, give bedside medication, and assign a ward and bed, with occupancy updating live.",
    journeyStep5Title: "Discharge & follow-up",
    journeyStep5Body:
      "Confirm clearances, generate a summary, and schedule the follow-up.",

    // Showcase — live board
    showcaseBoardEyebrow: "Live board",
    showcaseBoardTitle: "See where every patient is, at a glance",
    showcaseBoardBody:
      "One board groups everyone by where they are in their care, from intake to discharge. No more walking the wards to find out who is still waiting.",
    showcaseBoardPoint1: "Colour-coded by care stage and triage acuity",
    showcaseBoardPoint2: "Tap any patient for their complete clinical record",
    showcaseBoardPoint3: "Updates the moment a colleague makes a change",

    // Showcase — reports & beds
    showcaseReportsEyebrow: "Reports & beds",
    showcaseReportsTitle: "The numbers your review board asks for, ready to export",
    showcaseReportsBody:
      "Admissions, outpatients, bed occupancy, length of stay and outcomes, all counted automatically and exported to PDF or Excel in two clicks.",
    showcaseReportsPoint1: "Live bed and ward occupancy",
    showcaseReportsPoint2: "Weekly and monthly summaries",
    showcaseReportsPoint3: "One-click PDF and Excel export",

    // Feature grid
    featuresTitle: "Everything the hospital does, in one place",
    featuresSubtitle:
      "One system that replaces the admission register, the bed whiteboard and the reporting spreadsheet.",
    f1Title: "The whole patient journey",
    f1Body:
      "From nurse intake and triage, through the doctor's consultation, tests and results, prescriptions and bedside medication, to admission, beds and discharge.",
    f2Title: "Every role, one record",
    f2Body:
      "Reception, nurses, doctors, lab, pharmacy and administration each see exactly what their job needs, working from the same up-to-date record.",
    f3Title: "Beds & reports that stay live",
    f3Body:
      "See which beds are free, count admissions and outpatients, and export weekly and monthly reports for the review board.",
    f4Title: "Bilingual and works offline",
    f4Body:
      "Full French and English, and a record that keeps working when the internet drops. Changes sync automatically when it returns.",
    f5Title: "Your data, isolated and secure",
    f5Body:
      "Every hospital's records are fully separated. Staff only ever see their own hospital, enforced right at the database, not just the screen.",
    f6Title: "Set up in minutes",
    f6Body:
      "Create your hospital, add departments, wards, beds and staff, and start admitting patients the same day. No installation, no servers.",

    // Roles band
    rolesTitle: "Built for everyone on the floor",
    rolesSubtitle:
      "Each role sees exactly what their job needs, with no clutter and no training manual.",
    roleReception: "Reception",
    roleReceptionDesc: "Register arrivals and find any patient fast.",
    roleNurse: "Nurses",
    roleNurseDesc: "Triage, vitals, bedside care and handover.",
    roleDoctor: "Doctors",
    roleDoctorDesc: "Consults, orders, diagnoses and prescriptions.",
    roleLab: "Lab & imaging",
    roleLabDesc: "Work the order queue and attach results.",
    rolePharmacy: "Pharmacy",
    rolePharmacyDesc: "Dispense and confirm medication readiness.",
    roleAdmin: "Administration",
    roleAdminDesc: "Staff, beds, reports and oversight.",

    ctaTitle: "Bring your hospital online today",
    ctaBody:
      "Create your hospital account in a minute. Your data stays yours, and every hospital is fully isolated.",
    ctaNote: "Free during your trial, no card required.",
    footerNote: "The hospital's own operational record.",
    privacy: "Privacy",
    terms: "Terms",
  },

  followUp: {
    title: "Post-discharge follow-ups",
    subtitle:
      "Work the calls and check-ins scheduled automatically when a patient is discharged.",
    loading: "Loading follow-ups…",
    pendingCount: "pending",
    // Sections
    dueSection: "Due",
    upcomingSection: "Upcoming",
    doneSection: "Done",
    // Kind labels
    kindCall: "Phone call",
    kindTeleCheckin: "Tele check-in",
    kindSummaryDelivery: "Summary delivery",
    // Row details
    dueOn: "Due {date}",
    overdue: "Overdue",
    completedOn: "Completed {date}",
    noPhone: "No phone on file",
    // Actions
    markDone: "Mark done",
    cancelTask: "Cancel",
    // Empty states
    allClear: "No follow-ups pending",
    allClearHint:
      "New tasks are scheduled automatically each time a patient is discharged.",
    noneUpcoming: "Nothing scheduled ahead.",
    noneDone: "No completed follow-ups yet.",
    // Errors (notify keys)
    actionFailedTitle: "Could not update the follow-up task",
    actionFailedBody:
      "The change was not saved on this device. Try again; if it keeps happening, contact your administrator.",
  },

  offline: {
    title: "You're offline",
    body: "This page hasn't been cached yet. Pages you've already visited still work offline, and any changes you make are saved on this device and will sync once you're back online.",
    backToBoard: "Back to the board",
  },

  auth: {
    backHome: "Back to home",
    backendMissing: {
      title: "This copy of CareFlow is not connected to a server",
      body: "Sign-in is unavailable because the server settings are missing. If you run this app, copy .env.example to .env.local and fill in the Supabase values, then restart. If you're a staff member, contact your administrator.",
    },
    login: {
      title: "Sign in",
      subtitle: "Enter your username and password to continue.",
      usernameLabel: "Username",
      usernamePlaceholder: "e.g. admin",
      passwordLabel: "Password",
      passwordPlaceholder: "Your password",
      submit: "Sign in",
      signingIn: "Signing in…",
      error: "Incorrect username or password.",
      noAccount: "New hospital?",
      signUpLink: "Create an account",
      demoHint:
        "Demo account. Username: admin · password: CareFlow2026 (Douala General Hospital).",
      ownerDivider: "Hospital owner",
      ownerHint:
        "Signed up with Google or your email? Continue here. Staff use the username + password above.",
    },
    owner: {
      googleButton: "Continue with Google",
      googleSubmitting: "Redirecting to Google…",
      or: "or",
      emailLabel: "Email address",
      emailPlaceholder: "you@hospital.org",
      sendCode: "Email me a code",
      sending: "Sending…",
      codeSentTo: "Enter the 6-digit code we sent to {email}.",
      codeLabel: "Verification code",
      codePlaceholder: "123456",
      expiresIn: "Expires in {time}",
      expired: "Code expired",
      verify: "Verify & continue",
      verifying: "Verifying…",
      resend: "Resend code",
      changeEmail: "Use a different email",
      googleError: "Couldn't start Google sign-in. Please try again.",
      sendError: "Couldn't send the code. Check the address and try again.",
      verifyError: "That code is invalid or expired. Please try again.",
    },
    signup: {
      title: "Create your hospital",
      subtitle:
        "First, verify your identity with Google or your email. Then you'll set up your hospital.",
      haveAccount: "Already have an account?",
      loginLink: "Sign in",
      trialNote:
        "Starts on a free trial. Your hospital's data is completely isolated from every other hospital.",
    },
    onboarding: {
      title: "Set up your hospital",
      subtitle:
        "You're verified. Tell us about your hospital to finish. You can add departments, wards, beds and staff right after.",
      verifiedAs: "Verified as {email}",
      sectionHospital: "Your hospital",
      hospitalName: "Hospital name",
      hospitalNamePlaceholder: "e.g. Douala General Hospital",
      region: "Region / city",
      regionPlaceholder: "e.g. Littoral, Douala",
      contactEmail: "Contact email",
      contactPhone: "Contact phone",
      invalidEmail: "Enter a valid email address.",
      invalidPhone: "Enter a valid phone number for the selected country.",
      adminName: "Your full name",
      adminNamePlaceholder: "e.g. Dr. Amina Okafor",
      submit: "Create hospital",
      creating: "Creating…",
      error: "Could not create your hospital. Please try again.",
      signOut: "Not you? Sign out",
    },
    callback: {
      title: "Signing you in…",
      subtitle: "Completing your secure sign-in. This only takes a moment.",
      error: "We couldn't complete sign-in. Please try again.",
      backToLogin: "Back to sign in",
    },
  },

  account: {
    menu: "Account",
    signedInAs: "Signed in as",
    role: "Role",
    hospital: "Hospital",
    signOut: "Sign out",
  },

  subscription: {
    trialBadge: "Free trial",
    trialMessage:
      "You're on a free trial. All features are unlocked while you evaluate CareFlow.",
  },

  notifications: {
    title: "Notifications",
    ariaLabel: "Notifications",
    empty: "You're all caught up",
    emptyHint: "Updates about your patients will show up here.",
    markAllRead: "Mark all read",
    unreadCount: "{count} unread",
    justNow: "just now",
    minutesAgo: "{count}m ago",
    hoursAgo: "{count}h ago",
    daysAgo: "{count}d ago",
    enablePush: "Enable push alerts",
    pushEnabled: "Push alerts on",
    pushBlocked: "Push is blocked in your browser settings",
    pushUnsupported: "Push isn't supported on this device",
    pushPrompt: "Get alerted even when CareFlow is closed.",
  },

  settings: {
    title: "Hospital Settings",
    subtitle: "Configuration that applies to everyone in this hospital.",
    adminOnly: "Only administrators can view and change hospital settings.",
    loading: "Loading settings…",
    loadFailed: "Could not load settings. Check your connection and reload.",
    notifTitle: "Notifications",
    medRemindersLabel: "Medication due-soon reminders",
    medRemindersHint:
      "Shortly before a scheduled dose is due, ward nurses get an alert with how many medications are coming due. It is a count only: no patient names or drugs ever appear on a lock screen.",
    leadLabel: "Lead time",
    leadHint: "How far ahead of the due time the reminder goes out.",
    leadOption: "{count} minutes before",
    escalationLabel: "Overdue-dose escalation",
    escalationHint:
      "If a dose is still not recorded past its due time, the ward nurses and the attending doctor get an alert. Count only, like the reminders.",
    escalationAfterLabel: "Escalate after",
    escalationAfterHint: "How long past the due time before it escalates.",
    escalationOption: "{count} minutes past due",
    eventsTitle: "Event notifications",
    eventsHint:
      "Turn off any event your staff should not be alerted about. Applies to the whole hospital; devices pick up changes at their next sign-in.",
    events: {
      consultationCreated: "Consultation written",
      orderCreated: "Test or imaging ordered",
      resultRecorded: "Result recorded",
      prescriptionCreated: "Medication prescribed",
      vitalsRecorded: "Vitals recorded",
      marException: "Dose held, refused or missed",
      careplanEscalation: "Care plan escalation to doctor",
      careplanAcknowledged: "Care plan entry acknowledged",
      visitRegistered: "Patient registered",
      admissionCreated: "Patient admitted",
      transferRecorded: "Patient transferred",
    },
    save: "Save changes",
    saving: "Saving…",
    saved: "Settings saved",
    savedBody:
      "Reminder rules apply within a minute; event changes reach other devices at their next sign-in.",
    saveFailed: "Could not save settings",
    saveFailedBody: "Check your connection and try again.",
  },
  ai: {
    assist: "AI Assist",
    suggestNextSteps: "Suggest next steps",
    reviewResults: "Review results",
    generating: "Thinking…",
    assessment: "Suggested assessment",
    differential: "Possible conditions to consider",
    plan: "Suggested plan",
    tests: "Suggested tests",
    diagnoses: "Suggested diagnoses",
    medications: "Suggested medications",
    disposition: "Admission recommendation",
    dispositionNote:
      "Use the care stage controls below to admit, observe or discharge — nothing happens automatically.",
    dispo: {
      admit: "Consider admitting",
      discharge: "Consider discharging",
      observe: "Consider observation",
    },
    accept: "Accept",
    edit: "Edit",
    dismiss: "Dismiss",
    add: "Add",
    added: "Added",
    accepted: "Saved",
    insertedNote: "Added to the note — review it and press Save",
    prescribe: "Prescribe",
    prescribed: "Prescribed",
    primary: "Primary",
    why: "Why",
    sources: "Based on",
    confidence: {
      low: "Low confidence",
      moderate: "Moderate confidence",
      high: "High confidence",
    },
    med: {
      dose: "Dose",
      route: "Route",
      frequency: "Frequency",
      duration: "Duration",
    },
    insufficientData: "Not enough information yet to suggest this safely.",
    allergyWarning: "Conflicts with a recorded allergy — review before prescribing.",
    allergyAcknowledge: "I have reviewed this conflict and take responsibility.",
    unavailable: "AI is unavailable right now — you can continue manually.",
    offline: "AI needs an internet connection — you can keep working manually.",
    rateLimited: "The AI service is busy — wait a minute and try again.",
    needsConsultation: "Save the subjective / review of systems first.",
    needsResults: "No results recorded for this visit yet.",
    disclaimer:
      "AI suggestions are decision support only. The clinician is responsible for all decisions.",
    ask: {
      title: "Ask CareFlow",
      subtitle: "A read-only research assistant over your hospital's records.",
      patientTab: "This patient",
      cohortTab: "Across patients",
      placeholder: "Ask about this patient or a group of patients…",
      readOnlyNote: "Read-only — Ask CareFlow never changes records.",
      askButton: "Ask",
      selectPatient: "Search for a patient…",
      noPatient: "Pick a patient to ask about their record.",
      followUps: "Follow-up questions",
      queryPreview: "What was searched",
      rows: "Matching records",
      count: "{count} matching record(s)",
      empty: "No matching records.",
      cohortNote: "Cohort questions are available to doctors and admins.",
    },
  },
};

export type Messages = typeof en;
