 ...
 * `create type … as enum (…)` in `supabase/schema.sql`, and once as an
 ...
 *    the enum regex never sees them. The two that mirror TS unions
 *    (`follow_up_tasks.kind` / `follow_up_tasks.status`) are still guarded:
 *    a second regex parses their `check (col in (…))` lists and asserts
 *    parity against FOLLOW_UP_KINDS / FOLLOW_UP_STATUSES below.
 */
...
/** Extract every `check (<col> in ('a','b'))` member list from SQL. */
export function extractCheckInConstraints(sql: string): Map<string, string[]> {
  const checks = new Map<string, string[]>();
  const decl = /check\s*\(\s*(\w+)\s+in\s*\(([^)]*)\)\s*\)/gi;
  for (const m of sql.matchAll(decl)) {
    const members = [...m[2].matchAll(/'([^']*)'/g)].map((q) => q[1]);
...
export function diffEnum(
  ...
  if (missingInTs.length > 0 || extraInTs.length > 0) {
    return (
      `Enum "${name}" drifted between supabase/schema.sql and types/healthcare.ts:` +
...
describe("schema.sql ↔ types/healthcare.ts enum parity", () => {
  ...
  it("finds the expected enum declarations in schema.sql (regex sanity)", () => {
    ...
    expect(pgEnums.size).toBeGreaterThanOrEqual(20);
  ...
  it("follow-up kind/status check-constraints match their TS unions", () => {
    const checks = extractCheckInConstraints(sql);
    // Both constraints live on follow_up_tasks; the column names are the only
    // `check (col in (…))` constraints in the schema today.
